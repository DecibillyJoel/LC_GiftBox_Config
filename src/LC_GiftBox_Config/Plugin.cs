...
public class Plugin : BaseUnityPlugin
{
  ...
  internal readonly Harmony harmony = new("DBJ.SprintWeightConfig");

  public static void Log(LogLevel logLevel, string logMessage)
  {
    PluginLogger.Log(logLevel, $"[{LCMPluginInfo.PLUGIN_NAME}.{LCMPluginInfo.PLUGIN_VERSION}] {logMessage}");
  }
  ...
  private void ValidateConfigAndApplyPatches(object? eventSender = null, SettingChangedEventArgs? eventArgs = null)
  {
    ...
    if (spawnStoreItemChance.Value == 0 && spawnScrapChance.Value == 0 && spawnNothingChance.Value == 0 && doNothingChance.Value == 0) {
  ...
  private void Awake()
  {
    ...
    spawnStoreItemChance = Config.Bind("Gift Box - Behavior Selection", "Chance to select a store item (Weight)", 30, new ConfigDescription("The selection weight of a gift box selecting a store item. 0 = will not happen, larger selection weight = more likely to happen\n\n[Vanilla Value: 0]", new AcceptableValueRange<int>(0, 100), []));
    spawnScrapChance = Config.Bind("Gift Box - Behavior Selection", "Chance to select a scrap item (Weight)", 65, new ConfigDescription("The selection weight of a gift box selecting a scrap item. 0 = will not happen, larger selection weight = more likely to happen\n\n[Vanilla Value: 100]", new AcceptableValueRange<int>(0, 100), []));
    spawnNothingChance = Config.Bind("Gift Box - Behavior Selection", "Chance to select no item (Weight)", 5, new ConfigDescription("The selection weight of a gift box spawning nothing. 0 = will not happen, larger selection weight = more likely to happen\n\n[Vanilla Value: 0]", new AcceptableValueRange<int>(0, 100), []));
    doNothingChance = Config.Bind("Gift Box - Behavior Selection", "Chance to leave gift box unmodified (Weight)", 0, new ConfigDescription("The selection weight of a gift box not being modified by this mod, i.e. so another gift box mod's effects can function instead. 0 = will not happen, larger selection weight = more likely to happen\n\nIf you do not have any other gift box mods that function by transpiling OpenGiftBoxServerRpc(), I recommend leaving this value at 0. Otherwise, I recommend setting their probability values to 100% and this probability value to the weight you'd like to assign to the other mod, so that whenever this mod selects this hands-off behavior, the other mod's functionality will have a 100% chance to occur rather than simply using vanilla behavior", new AcceptableValueRange<int>(0, 100), []));
    ...
    scrapValueAdditionMin = Config.Bind("Gift Box - Scrap Item Value", "Minimum scrap item value addition", 25, new ConfigDescription("The minimum possible value of the multiplier applied to the selected scrap item's scrap value\n\n[Vanilla Value: 25]", new AcceptableValueRange<int>(-1000, 1000), []));
    scrapValueAdditionMax = Config.Bind("Gift Box - Scrap Item Value", "Maximum scrap item value addition", 35, new ConfigDescription("The maximum possible value of the multiplier applied to the selected scrap item's scrap value\n\n[Vanilla Value: 35]", new AcceptableValueRange<int>(-1000, 1000), []));
    ...
    scrapValueMultiplierMin = Config.Bind("Gift Box - Scrap Item Value", "Minimum scrap item value multiplier (%)", 150, new ConfigDescription("The minimum possible value of the multiplier applied to the selected scrap item's scrap value\n\n[Vanilla Value: 100%]", new AcceptableValueRange<int>(0, 1000), []));
    scrapValueMultiplierMax = Config.Bind("Gift Box - Scrap Item Value", "Maximum scrap item value multiplier (%)", 200, new ConfigDescription("The maximum possible value of the multiplier applied to the selected scrap item's scrap value\n\n[Vanilla Value: 100%]", new AcceptableValueRange<int>(0, 1000), []));
